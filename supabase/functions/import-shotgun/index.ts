...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use Firecrawl search to find Shotgun events
    const searchQuery = `site:shotgun.live ${keyword} ${city} 2025 2026`;
    console.log("Step 1: Searching Shotgun:", searchQuery);
    ...
    console.log(`Step 1 done. Found ${results.length} Shotgun results`);

    if (results.length === 0) {
      return new Response(
        JSON.stringify({ success: true, city, imported: 0, skipped: 0, total_found: 0, message: "No Shotgun results found" }),
    ...
    const aiRes = await fetch(AI_GATEWAY, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You extract event data from Shotgun.live search results. Return ONLY valid JSON, no markdown code fences."
          },
          {
            role: "user",
    ...
    console.log(`Found ${events.length} Shotgun events for ${city}`);
    ...
    for (const ev of events) {
      ...
      const externalId = ev.url || `shotgun-${city}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
