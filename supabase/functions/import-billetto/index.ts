...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use Firecrawl search to find Billetto events
    const searchQuery = `site:billetto.co.uk OR site:billetto.dk OR site:billetto.se OR site:billetto.fi OR site:billetto.no ${keyword} ${city} 2025 2026`;
    console.log("Step 1: Searching Billetto:", searchQuery);
    ...
    console.log(`Step 1 done. Found ${results.length} Billetto results`);

    if (results.length === 0) {
      return new Response(
        JSON.stringify({ success: true, city, imported: 0, skipped: 0, total_found: 0, message: "No Billetto results found" }),
    ...
    const aiRes = await fetch(AI_GATEWAY, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You extract event data from Billetto search results. Return ONLY valid JSON, no markdown code fences."
          },
          {
            role: "user",
    ...
    console.log(`Found ${events.length} Billetto events for ${city}`);
    ...
    for (const ev of events) {
      ...
      const externalId = ev.url || `billetto-${city}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
