...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use Firecrawl search to find DICE.fm events
    const searchQuery = `site:dice.fm ${keyword} ${city} 2025 2026`;
    console.log("Step 1: Searching DICE.fm:", searchQuery);
    ...
    console.log(`Step 1 done. Found ${results.length} DICE results`);

    if (results.length === 0) {
      return new Response(
        JSON.stringify({ success: true, city, imported: 0, skipped: 0, total_found: 0, message: "No DICE results found" }),
    ...
    const aiRes = await fetch(AI_GATEWAY, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You extract event data from DICE.fm search results. Return ONLY valid JSON, no markdown code fences."
          },
          {
            role: "user",
            content: `Extract all individual events from these DICE.fm search results for ${city}. Return a JSON object with an "events" array. Each event should have: title, date (ISO 8601 format like 2025-03-15T20:00:00), location (venue name if found), price (string like "Free" or "£15"), url (the dice.fm URL), image_url (if found). Only include actual events with real titles. Skip duplicates. Content is between [START] and [END] markers.\n\n[START]\n${sanitizeForAI(combined)}\n[END]`
          }
        ],
        temperature: 0,
        max_tokens: 4000,
    ...
    console.log(`Found ${events.length} DICE events for ${city}`);
    ...
    for (const ev of events) {
      ...
      const externalId = ev.url || `dice-${city}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
